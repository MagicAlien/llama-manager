...
| v6.6 | v6.7 | |
|---|---|---|
| — | **T-049** the install surface: an update replaces its own build, and any build can be installed | new: owner-commissioned on 20 September 2026 while testing T-041's build — the per-build button reads "install" while it updates, it adds beside the build it was pressed on instead of replacing it, and the app could not install any build but the backend already installed |
...
### T-049 — Installing llama.cpp is the manager's own job: an update replaces the build it was pressed on, and any build can be installed `[dep: T-020, T-021, T-022, T-024, T-040]`

The app is named for managing llama.cpp and is the one thing it cannot do: the button that says `Install new build` sits *inside each installed build's card*, and what it can install is decided by that card — `VersionManagement.tsx` hands it `getLatestForBackend(build.backend)`, which filters the release list to the **same backend** (same CUDA major). So a machine holding only a CPU build can only ever get another CPU build, while the app's own `core::backend_selection::select_backend` — whose documented consumer is the UI, and which still carries `#[allow(dead_code)]` — knows that this machine's Blackwell card needs **CUDA 13** and that anything below it is a silent CPU fallback (`PLAN.md` §2.2). Two more facts make the current button wrong as well as narrow: `install_runtime` adds a **row beside** the old build instead of replacing it, and the card's own copy already reads `Update available` / `Latest available: b11064` — only the button still says "install". The button's position promises that *this* version updates itself; the behaviour contradicts both the promise and the surrounding text.

**What an update is, by the owner's decision** (*"Quello che si dovrebbe fare quindi è cambiare da 'install new build' a 'update', e installare la build più recente al posto di quella da cui si è premuto il tasto (esempio: da b10883 a latest available)"*): pressing `Update to <tag>` installs the newest build **of that build's backend** and **removes the build it was pressed on**. The backend is not repeated in the label — the card already carries it as real data (`backendLabel(build.backend)`).

**What installing is, for everything else**: a separate affordance — not a second meaning for the same button — opens a picker with two **dropdowns**, backend and version, both **prefilled** with the recommendation (`select_backend` for the current machine: CUDA 13 here) and its newest candidate, so an empty form is never shown and no version has to be typed. Selecting a backend whose candidate list is empty is not offered at all, and a Blackwell machine whose release list carries no CUDA ≥ 13 asset is refused **with the silent-CPU-fallback reason**, never quietly downgraded. Installing a build of a backend other than the installed one is the point of the feature, so a CPU-only catalogue must offer CUDA 13 (and Vulkan) as selectable.

**Three mechanical consequences the design has to carry.** (1) The picker needs candidates that do not exist in the app today: `gh_releases::check_for_updates` deliberately returns only the **newest release per backend** (T-020) and stays exactly that, because T-024's badge reads it — a second reader returns **all** Windows x64 builds from the releases window the API allows (its list endpoint stops at 1000 results, `PROGRESS.md` F-008), each candidate carrying tag, backend, asset name, size and publication date, with the window bound asserted and the cap documented where it is applied. (2) Every candidate is a **real published asset**: a tag with no asset for the chosen backend is not a candidate, so no list entry can produce a 404 at install time. (3) Removing a runtime is refused while the server is not `Stopped` and the last remaining build cannot be removed (`CONTRACTS.md` §2), so the update runs **install → activate → remove**, in that order and no shortcut: activation happens only when the replaced build was the active one (**the replacement inherits activation** — the alternative leaves an updated install with nothing active, and the next `start_server` failing is not a defensible outcome of pressing "update"), and the old build is removed only after its successor is registered *and* active. A failure at any step leaves the catalogue and the active flag exactly as they were and names the step that failed; no code path may remove a build before its replacement is usable.

**An update is reversible — the owner's requirement** (*"Se la funzione è di tipo 'aggiorna e sostituisci' allora deve dare la possibilità di ritornare all'istallazione precedente"*), and the rollback lives **in the build's own card, not in a row of its own** (*"non creare una riga apposita per il rollback, basta un tasto nella scheda della build"*): the version list keeps rendering **one card per backend**, and the card produced by an update carries `Roll back to <previous tag>` — one button, whose label names the exact tag it restores. The replaced build is therefore **retained, not deleted**: its `runtimes` row keeps its `install_path` and its `verified_flags_json` (that capture is what makes it launchable again without re-probing a build nobody has run), and it is kept out of the card list by a new **nullable marker column** added by a new forward-only migration file `0002_*.sql` plus its entry in `MIGRATIONS` — `0001_init.sql` is never edited — and *not* by filtering on `is_active`, because a catalogue whose builds have not been activated yet has no active row at all and would render an empty list. At most **one** retained build per backend: the following update removes the older one through the existing removal path, so the disk cost is bounded at one extra build per backend and the list cannot grow with every update; and **removing a build removes its retained companion too**, so no file survives on disk that no screen can free. A rollback is a **swap** of that pair — the retained build becomes the card and the build it replaces becomes the retained one, so the button that read `Roll back to b10883` reads `Roll back to b11064` afterwards, and the single-active invariant (`idx_runtimes_single_active`) holds throughout. Re-installing the old tag on demand was considered and rejected as the mechanism: it turns a rollback into a download of hundreds of megabytes at the moment the machine may be broken, it needs the network exactly when the user is least able to trust it, and a tag older than the release-list window (`PROGRESS.md` F-008) is not discoverable from the list at all.

*Acceptance:* against a fixture release list and a stubbed installer (no network, no GPU) —

- **Update replaces**: pressing update on the one installed build leaves exactly one row for that backend, whose tag is the newest candidate of that backend, and the replaced tag is absent. Asserted over a fixture list with several builds per backend.
- **Activation follows the update**: when the replaced build was active, the successor is active afterwards and no other row is; when the replaced build was **not** active, the active flag is untouched. Both branches asserted.
- **Refused while the server is not `Stopped`**: the action is refused with the reason displayed, and neither an install nor a removal is attempted (asserted on the calls the stubbed boundary received, not on the message alone).
- **A failed install changes nothing**: a stubbed install failure leaves the row set and the active flag byte-identical to before, and a failure at the activation step leaves the old build in place (asserted both ways) — the class of bug where "update" ends with no usable build.
- **The picker's defaults are the recommendation, not a constant**: for a Blackwell fixture profile the prefilled backend is CUDA 13 and the prefilled version its newest candidate; for a no-GPU profile it is CPU with the poor-performance warning shown; for a Blackwell profile whose list has only CUDA 12 the picker refuses with the silent-CPU-fallback reason rather than offering it. Table-driven over the three profiles.
- **Any backend is installable from any installed set**: with only a CPU build installed, the CUDA 13 candidate is selectable and installable (asserted on the fixture list), which is the behaviour that does not exist today.
- **No invented candidates**: every entry the picker offers appears in the reader's output with a real asset name for its backend; a tag whose asset is absent from the window never appears.
- **The release reader keeps the old client intact**: `check_for_updates`'s output is unchanged for the badge, and the new reader's window bound is asserted for a fixture response at the cap.
- **Rollback is a button on the build's card, and one card per backend**: after an update the screen renders exactly one card for that backend — the retained build is not a card — its text names the previous tag, and the button carries that tag in its own label; asserted on the rendered output and not on the data alone.
- **Rollback is a swap, offline and instant**: pressing it leaves the retained build active and the build it replaced retained in its place, with the **releases client never called** and no install call attempted (asserted on the calls the stubbed boundary received, not on the absence of an error).
- **Retention is bounded, and nothing is left unfreeable**: two consecutive updates leave exactly one retained row for that backend, the older one removed **with its files** (asserted on the filesystem, not only on the row count), and removing the visible build removes its retained companion with it.
- **The card list is not filtered by `is_active`**: a catalogue whose builds have never been activated still renders its cards — asserted with zero active rows and a non-empty list, the regression the marker column must not introduce.
- **A failed rollback changes nothing**, exactly like a failed update: the row set, the marker and the active flag are unchanged, and the action is refused while the server is not `Stopped` with the reason displayed.
- **Copy**: no internal task ids, no flag names and no jargon in any new string; the update action names what it will replace (`Update to b11064`), does not repeat the backend, and the two actions (update this build / install a build) are distinguishable in the UI without a manual.
- **Wiring**: the new reader's IPC command is registered in **both** `ipc/mod.rs` and `main.rs`'s `generate_handler!`, and has its `docs/CONTRACTS.md` §4 row naming this task (`AGENTS.md` invariant 9: `src/lib/types.ts` is regenerated, never hand-edited).
- **Demonstrated, not asserted**: one real end-to-end update on this machine — `b10883` (cpu) → the newest CPU build — with the rows before and after, the removed row and the active flag recorded in the PR body; the CUDA path is exercised against the fixture list only, because downloading a CUDA asset is not a CI concern. T-024's two rules that this task consumes rather than relaxes are re-asserted unchanged: removal is still refused while the server is not `Stopped`, and the last build is still not removable.
