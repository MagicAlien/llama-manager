...
- **T-010's PROGRESS.md entry claims a clean `cargo fmt --check` from a real Windows run, but T-011's CI (rustfmt 1.88.0, the version `rust-toolchain.toml` pins) found 15 unformatted hunks in `core/env_probe.rs`, entirely in T-010's own code (T-011 PR #10 review).** Fixed forward in T-011 by applying CI's own diff (see In progress above) rather than investigated here — out of T-011's scope to chase down why the two runs disagreed. Possibilities not ruled out: the Windows run that produced T-010's claim used a different rustfmt than the one pinned (a local override, a stale `rustup` default, or a version installed before the pin took effect), or the claim was made without actually capturing the real command output. Worth checking, next time a task claims a real gate run, that the pinned toolchain version is confirmed (`rustup show` or equivalent) rather than assumed from `rust-toolchain.toml` alone.
