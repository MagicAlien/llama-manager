...
**T-011 — Health-check screen.** Branch `t-011-health-check-screen`, pushed to `origin`. **PR not yet opened** — this working session has push access to `origin` (`git push` over `github.com` succeeded) but no `api.github.com` access (`curl` to it timed out against the network allowlist, same as T-005/T-010's own note), no `gh` CLI, and no browser tool, so the PR itself needs a human to open it from `https://github.com/MagicAlien/llama-manager/pull/new/t-011-health-check-screen`. The PR body is drafted and ready to paste (see the branch's second commit message / the session that produced it).
...
**Not yet verified, because this session had no Rust toolchain** (same constraint as T-001/T-003/T-004/T-010): `cargo build`, `cargo clippy -- -D warnings`, `cargo fmt --check`, `cargo test` against the `main.rs`/`ipc/mod.rs`/`core/env_probe.rs` changes. These are small, mechanical (one `invoke_handler` line, two attribute removals) and checked only by reading, not by a real compiler. First real evidence will be CI on the opened PR or a real Windows run.

**Next session or human action:** open the PR from the link above using the drafted body, let CI (or a real Windows checkout) run, then either continue this task here with the results or report them back so this entry and the PR checklist can be finished honestly — several PR checklist boxes are deliberately left unchecked pending that evidence (`AGENTS.md` §5).
