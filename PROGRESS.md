...
**Update:** the owner then ran `npm run generate-types` for real (real `cargo test`, 39/39 `export_bindings_*` tests passed — first real confirmation the crate compiles and every type actually derives `TS`/`Serialize`/`Deserialize`; the 14 round-trip tests were correctly filtered out by name, not run by this command). Two real findings, both fixed:
1. The hand-transcribed `src/lib/types.ts` differed from real `ts-rs` output in exactly the ways F-002 flagged as uncertain: quoted object keys (`"kind"` not `kind`), a trailing comma after every field including nested inline objects, and Rust doc comments carried over as JSDoc. None of that is a shape problem — replaced the committed file with the real captured output (owner-provided diff), so this part of D-... is now empirically confirmed rather than reasoned from docs.
2. A real one: `ServerProps.raw` (`serde_json::Value`) came out as `raw: JsonValue` — a type name ts-rs's `serde-json-impl` feature emits but does not define anywhere in the exported set (`wrote 39 types` — no fortieth `JsonValue.ts`). Concatenated into one file, that's a dangling reference `tsc` would reject. Fixed with `#[ts(type = "unknown")]` on the field, which needs no such definition; the `serde-json-impl` ts-rs feature is no longer enabled at all. Also fixed `scripts/generate-types.mjs` to strip ts-rs's per-file "This file was generated by ts-rs..." banner on collection (this session's own banner already says it once) — it was harmless noise this run but would duplicate 39 times on every future regeneration otherwise.
...
**Update:** owner ran `npm run generate-types` a third time; still 39/39 passing, but `src/lib/types.ts` still shows as stale — and this diff's changed lines render as *visually identical* text on both sides (`-export type AppSettings = {` / `+export type AppSettings = {`), which means the real difference is whitespace a pasted `git diff` in a chat log doesn't reliably convey (trailing spaces, or possibly the `LF will be replaced by CRLF` warning above it pointing at a line-ending mismatch). The previous fix (removing the blank line after each header) was a real, correctly-identified difference, but chasing this next layer by eye is guessing, not verifying — exactly what this session is trying not to do. Stopped guessing: `.github/workflows/ci.yml` now uploads the freshly generated `src/lib/types.ts` as a build artifact (`generated-types-ts`) on every run of `jobs.gates`, unconditionally. Next step is to take that artifact's file directly, byte for byte, rather than reconstruct it from a diff a third time.

**Update:** owner downloaded the `generated-types-ts` CI artifact and shared it directly. Diffed byte for byte against the committed file: the real generator's output has a trailing space after the opening `{` on any multi-field type whose first field carries a doc comment, and after any field immediately followed by a doc comment (e.g. `export type AppSettings = { ` — note the space — then a line break into `/**`). Every hand-reconstruction this session did (including the one that pasted the owner's own diff) dropped that space, and the blank-line fix's blanket `.rstrip()` on every line made the same mistake more thoroughly rather than fixing it. **`src/lib/types.ts` is now the artifact's exact bytes, copied in directly, not re-derived** — confirmed identical with `diff`, no transformation applied. `scripts/generate-types.mjs` needed no change: it never stripped trailing whitespace to begin with, so the artifact it produced was correct all along — every mismatch traced back to a hand-transcription in this session, not to the script. `npx eslint .` / `npx tsc --noEmit` both still pass against the real file. `cargo build`/`clippy`/`test` (full suite) remain the only unconfirmed pieces.
