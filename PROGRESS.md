...
- **T-005** — Design system · branch `t-005-design-system`, no PR yet
  ...
  - **Blocked on push/PR access, not on anything else.** No GitHub credentials (no `gh`, no token, no SSH key) were available in this session's environment. The branch is committed locally with a clean tree; `.github/pull_request_template.md` is filled out completely (only boxes with real evidence checked; the PROGRESS.md box is explicitly left unchecked, per its own instruction, until a real PR number exists) and handed to the owner as a patch bundle, the same fallback `AGENTS.md`/`docs/WORKFLOW.md` already establish for a missing toolchain — applied here to a missing credential instead. This is *not* a Discrepancy (nothing about the documents or the task's design is in question) and not an Observation (it's not an out-of-scope problem noticed in passing) — it's this task's own completion blocked on an environment gap, recorded here per `docs/WORKFLOW.md` §4's own instruction not to move a task to Done before it is actually confirmed passing.
