...
Last updated: 6 August 2026 — T-010 (EnvironmentProbe) is **Done**, PR #9, squash-merged into `main` (commit `29d7456`). Full local gate green on a real Windows machine (`cargo fmt --check`, `cargo build`, `cargo clippy -- -D warnings`, `cargo test`, `npm run generate-types` — one real fixup, `clippy::manual_unwrap_or_default`). Agent had no Rust toolchain and no Windows machine this session; implemented against the best reading and handed to the owner as a patch on a pushed branch, same pattern as T-001/T-003/T-004 — `api.github.com` stayed blocked throughout (push works, the API doesn't), so the owner also opened and merged the PR, same as T-005. D-004, D-005, D-010 and D-011 remain open, owner decisions, not touched. Take T-011 next — lowest-numbered task whose dependencies are all `Done` and which is not `Blocked` (T-006 is still `Blocked` on D-005; T-011 depends on T-005 and T-010, both now `Done`).

---

## In progress

*(nothing — T-010 is Done, PR #9, full local gate green on a real Windows machine. Take T-011 next — lowest-numbered task whose dependencies are all `Done` and which is not `Blocked`: T-006 is still `Blocked` on D-005; T-011 depends on T-005 and T-010, both `Done`.)*
...
- **`main.rs` does not register `probe_environment` with Tauri's `invoke_handler` (T-010).** `ipc/mod.rs` exists and the command compiles (as far as a `tree-sitter` parse can tell — no real compiler this session), but nothing calls `tauri::generate_handler![...]` yet, so the frontend cannot invoke it. Deliberately left out of T-010's scope (`docs/WORKFLOW.md` §3: no orchestrator, no UI) rather than reached for because T-011 was sitting right there. Whichever task first needs to call it from the frontend — most likely T-011 — should wire `.invoke_handler(tauri::generate_handler![ipc::probe_environment])` into `tauri::Builder` in `main.rs`, and should remove the `#![allow(dead_code)]` T-010 added to `ipc/mod.rs` (and, if nothing else roots it by then, to `core/env_probe.rs`) once the command has a real caller.
