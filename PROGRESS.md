...
- **T-010 — EnvironmentProbe.** Branch `t-010-environment-probe`, pushed to `origin` (not yet on `main`). `core/env_probe.rs` (`NvmlProvider` trait, GPU/driver/CUDA-13/disk/endpoint `HealthCheck`s, full mocked-provider test suite) and `ipc/mod.rs` (new module — first command, `probe_environment`) are written and pass a `tree-sitter` grammar parse (syntax only; not a substitute for `rustc`) — **no Rust toolchain and no Windows machine this session** (see Observations), so nothing has run against a real compiler: no `cargo fmt`/`build`/`clippy`/`test`, no `npm run generate-types`. Two dependencies added to `Cargo.toml` (`nvml-wrapper`, `windows`, features expanded beyond the `PLAN.md` §3 parenthetical scope — justified in the PR per `AGENTS.md` §4) that `Cargo.lock` has not been regenerated against; expect that regeneration, not just new-code fixups, on the first real `cargo build`. `api.github.com` is blocked here the same way T-005 found it blocked (`git push`/`git ls-remote` over `github.com` both work); the PR itself needs opening via the compare link `git push` reports, same as T-005. **Do not mark T-010 `Done` until a real toolchain run and a real merge both confirm it** — picking this up: read this entry, the branch, and the PR description already on it before doing anything else, per `docs/WORKFLOW.md` §1.
...
- **`npm run generate-types` should produce no diff to `src/lib/types.ts` from T-010, not the diff the task briefing assumed.** `GpuInfo`, `HealthCheck`, `EnvironmentReport`, `CheckStatus` and `Backend` were all already transcribed into `core/types.rs` with `#[ts(export)]` by T-002 and are already present in `src/lib/types.ts` — T-010 is the first to *construct* them outside a test, not the first to export them. Worth recording because the opposite assumption is an easy one to carry into the next session and then "fix" a diff that was never supposed to exist.
