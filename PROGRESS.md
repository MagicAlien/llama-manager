...
Last updated: 7 August 2026 — T-011 (Health-check screen) is **in progress**: implemented and pushed on branch `t-011-health-check-screen`, PR not yet opened (see In progress below — `api.github.com` stayed blocked this session too, same as T-005/T-010; push over `github.com` works, the API doesn't, and no `gh` CLI or browser tool was available either). Frontend gate green for real (`npm ci`, `npm run lint`, `npm run test` 8/8, `npm run build`). The Rust side (`main.rs` invoke_handler wiring) is implemented against the best reading only — no Rust toolchain this session, same constraint as T-001/T-003/T-004/T-010 — and needs a real `cargo build`/`clippy`/`fmt`/`test` run before it can be trusted. D-004, D-005, D-010 and D-011 remain open, owner decisions, not touched.

---

## In progress

**T-011 — Health-check screen.** Branch `t-011-health-check-screen`, pushed to `origin`. **PR #10 opened by the owner** (this session still has no `api.github.com`/`gh`/browser access — `curl` to `api.github.com` timed out against the network allowlist, same as T-005/T-010's own note — so the owner opened it from the compare link, same pattern as before). **Not yet merged.**

Implemented: `src/screens/Runtime.tsx` replaced (T-005's placeholder is gone) with the traffic-light/remediation/manual-refresh view `docs/TASKS.md` T-011 asks for, consuming `probeEnvironment()` (T-010) and the generated types as-is — no redefinition (`AGENTS.md` invariant 9). `src/screens/Runtime.test.tsx` proves each `CheckStatus` renders with a visually distinct class (`bg-pass`/`bg-warn`/`bg-destructive`), remediation shown only for Warn/Fail, and refresh re-invoking `probeEnvironment()` — reusing `src/App.test.tsx`'s hash + `hashchange` pattern, driven by TypeScript equivalents of T-010's four `core/env_probe.rs` fixture profiles (Blackwell, Ada, driver-too-old, no-GPU), with a `disk_space` Warn and a second `endpoint_bindable` Fail folded in (those two checks have their own independent Warn/Fail unit tests in `env_probe.rs`, not one of the four named GPU profiles, so this isn't an invented fifth profile). `main.rs` now registers `ipc::probe_environment` with `tauri::generate_handler!`, closing the Observation T-010 left naming this task as the likely place to do it; the `#![allow(dead_code)]` T-010 added to `ipc/mod.rs` and `core/env_probe.rs` is removed accordingly, since the command now has a real caller reachable from the binary's own entry point. Two new design tokens, `pass` and `warn`, were added to `tailwind.config.js` (the one tokens file, T-005) — nothing in the existing palette covered a three-way status colour, and `Fail` reuses the existing `destructive` token rather than duplicating red under a new name.

**Verified locally, real runs:** `npm ci`, `npm run lint` (clean), `npm run test` (8/8, including the four new `Runtime.test.tsx` cases), `npm run build` (`tsc` + `vite build`, clean — confirms `bg-pass`/`bg-warn` actually compiled into the output CSS, not just referenced).

**Real CI/Windows evidence so far, two fixups — this session still has no Rust toolchain, so both were applied from the owner's relayed output, not derived locally:**

1. Owner's real `cargo build` (Windows) reported one warning: `NvmlProvider::device_telemetry` never used — the module-wide `#![allow(dead_code)]` T-010 added (and this PR removed) had been masking it too, not just the "no real caller" issue the removal was meant to fix. Fixed by scoping `#[allow(dead_code)]` to that one trait method instead (it's reserved for a future GPU telemetry view, T-060/T-061), rather than reintroducing the module-wide allow.
2. CI's `cargo fmt --check` (rustfmt 1.88.0, pinned by `rust-toolchain.toml`) failed with 15 hunks in `core/env_probe.rs` — all in T-010's original code, none in the lines this PR actually added. Applied verbatim from the CI-reported diff. **Worth a look independently of this fix:** T-010's own PROGRESS.md entry claims a clean `cargo fmt --check` from a real Windows run at the time; whatever produced that result didn't match what CI's pinned toolchain produces on the same file now. Not chased down here (out of this task's scope, `docs/WORKFLOW.md` §3) — recorded in Observations below instead.

Still unverified: `cargo clippy -- -D warnings`, `cargo fmt --check` (re-run after the fixup above), `cargo test` — awaiting the next CI run or owner report.

**Next session or human action:** watch PR #10's CI (or ask the owner to re-run the local gate) after the fixups above; once every check is green *and the PR is actually merged*, close this out with the real merge commit and finish the PR checklist honestly (`AGENTS.md` §5) — several boxes are deliberately left unchecked until then.
